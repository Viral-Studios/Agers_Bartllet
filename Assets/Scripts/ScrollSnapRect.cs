...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler {
    ...
    public GameObject Padrebotones;
    ...
    public void LerpToPage(int aPageIndex) {
        ...
        //Padrebotones.transform.GetChild(aPageIndex).GetComponent<Image>().color = colorOn;

        /*for (int i = 0; i < 5; i++){
            if (i != aPageIndex && Padrebotones.transform.GetChild(i).GetComponent<Image>().color == colorOn){
                Padrebotones.transform.GetChild(i).GetComponent<Image>().color = ColorOff;
            }
        }*/
    }
    ...
    public void AparDesapaOn (int num){

        for (int i = 0; i < contenidoss.transform.childCount; i++){

            if (i == num){
                contenidoss.transform.GetChild(num).gameObject.SetActive(true);
            }else{
                if (contenidoss.transform.GetChild(i).gameObject.activeSelf){
                    contenidoss.transform.GetChild(i).gameObject.SetActive(false);
                }
            }
        }
    }
